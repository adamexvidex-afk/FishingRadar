...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const adminEmails = (Deno.env.get("ADMIN_EMAIL") || "").split(",").map(e => e.trim().toLowerCase());
    ...
    const { batchSize = 2 } = await req.json().catch(() => ({}));
    ...
    if (!fish || fish.length === 0) {
      return new Response(JSON.stringify({ message: "All fish have illustrations", remaining: 0 }), {
